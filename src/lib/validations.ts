...
export const receiptFormSchema = z.object({
  ...
  business_number: z.string().max(15, 'GST/BN should be at most 15 characters').optional(),
  ...
  card_last_four: z.string().regex(/^\d{0,4}$/, 'Must be up to 4 digits').optional(),
  ...
  usage_type: z.enum(USAGE_TYPES as [string, ...string[]]).nullable(),
  business_use_percent: z.number().min(0).max(100),
  business_unit_id: z.string().optional(),
  
  capture_source: z.string().optional(),
